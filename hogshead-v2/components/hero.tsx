...
export function Hero() {
  return (
    <section id="top" className="relative min-h-screen overflow-hidden bg-[#061f25] pt-28 text-white">
      <video className="absolute inset-0 h-full w-full object-cover opacity-72" src="/images/Animated Background.mp4" autoPlay muted loop playsInline />
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(2,26,32,.96),rgba(2,34,40,.88)_42%,rgba(2,34,40,.46)),radial-gradient(circle_at_72%_36%,rgba(216,139,66,.20),transparent_26%),radial-gradient(circle_at_22%_30%,rgba(123,198,199,.20),transparent_30%)]" />
      ...
      >
        <div className="absolute inset-12 rounded-full border border-gold/18" />
        <div className="absolute inset-28 rounded-full border border-aqua/18" />
      </motion.div>
      ...
      <Shell className="relative z-10 grid min-h-[calc(100vh-7rem)] items-center gap-12 py-16 lg:grid-cols-[1fr_480px]">
        <Reveal>
          ...
          <p className="mt-7 max-w-2xl text-[18px] font-semibold leading-8 text-white/86 drop-shadow-[0_4px_18px_rgba(0,0,0,.30)]">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <a href="#simulator" className="rounded-full bg-white/94 px-6 py-3 text-[14px] font-extrabold text-deep shadow-soft">Run Aging Model</a>
        ...
        <Reveal delay={0.1} className="relative">
          ...
          <div className="relative overflow-hidden rounded-[38px] border border-white/24 bg-[#062b32]/72 p-5 shadow-[0_42px_130px_rgba(0,0,0,.44)] backdrop-blur-2xl">
            <div className="relative overflow-hidden rounded-[30px] border border-white/18 bg-[#082f36]/92 p-6">
              <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(2,34,40,.72),rgba(2,34,40,.96)),url('/images/Barrel-aging-copy-5.jpeg')] bg-cover bg-center opacity-92" />
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_68%_18%,rgba(216,139,66,.18),transparent_28%)]" />
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between gap-5">
                  ...
                  <div className="grid h-14 w-14 place-items-center rounded-full border border-white/24 bg-white/14 text-gold">
                  ...
                  {platformProof.map((item, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="group flex items-center gap-4 rounded-2xl border border-white/22 bg-[#06252b]/72 p-4 shadow-[0_12px_34px_rgba(0,0,0,.18)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-[#06343d]/86"
                      >
                        ...
                        <div>
                          ...
                          <span className="text-[12px] font-bold text-white/76">{item.text}</span>
                        </div>
                ...
                <div className="mt-6 grid grid-cols-3 gap-3">
                  {['1–21Y', '43–55%', 'TX / CA'].map((value, index) => (
                    <div key={value} className="rounded-2xl border border-white/18 bg-[#061f25]/76 p-4 text-center backdrop-blur-xl">
                      <strong className="block text-[20px] font-extrabold text-white">{value}</strong>
                      <span className="mt-1 block text-[9px] font-extrabold uppercase tracking-[0.14em] text-white/58">{index === 0 ? 'Inventory' : index === 1 ? 'ABV range' : 'Initial focus'}</span>
                    </div>
                  ))}
                </div>
              </div>
